  ...
  <WidgetCard :title="t('dashboard.widgets.services.title')" :icon="Calendar01Icon" :loading="isLoading">
    <div class="space-y-6">
      ...
      <div class="relative inline-block w-full">
        <select v-model="servicesFilter" class="w-full bg-slate-50/50 dark:bg-[#13161C]/60 border border-slate-200/60 dark:border-white/5 rounded-xl px-4 py-2 text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest outline-none cursor-pointer hover:bg-white dark:hover:bg-white/5 transition-all appearance-none">
      ...
      <div class="grid grid-cols-2 gap-2">
        <div class="bg-white/50 dark:bg-[#13161C]/60 rounded-xl border border-slate-200/60 dark:border-white/5 p-3 flex flex-col items-center group/stat transition-all duration-300 hover:scale-[1.05] shadow-sm">
          <p class="text-2xl font-black text-[#3b82f6] dark:text-[#5da6fc] tracking-tighter">{{servicesData.executing}}</p>
        ...
        <div class="bg-emerald-500/5 dark:bg-emerald-500/10 rounded-xl border border-emerald-500/20 dark:border-emerald-500/10 p-3 flex flex-col items-center group/stat transition-all duration-300 hover:scale-[1.05] shadow-sm">
          <p class="text-2xl font-black text-emerald-500 dark:text-emerald-400 tracking-tighter">{{servicesData.finished}}</p>
