<template>
  <div class="flex-1 flex flex-col gap-6 p-5 lg:p-8 overflow-y-auto custom-scrollbar bg-slate-50/80 dark:bg-transparent">
    <div class="max-w-7xl mx-auto w-full flex flex-col gap-6">
      ...
      <div 
        class="bg-white/40 dark:bg-[#1A1D24]/30 backdrop-blur-xl rounded-2xl border border-slate-200/80 dark:border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.05)] dark:shadow-[0_4px_30px_rgba(0,0,0,0.3)] flex flex-col"
        :class="allowOverflow ? '' : 'overflow-hidden'"
      >
        <!-- Tabla / Contenido Principal -->
        <div :class="allowOverflow ? 'block' : 'block overflow-x-auto'">
          <slot></slot>
        </div>
