...
const isSelected = (day: number) => {
  ...
  return day === selectedDate.value.getDate() && viewDate.value.getMonth() === selectedDate.value.getMonth() && viewDate.value.getFullYear() === selectedDate.value.getFullYear()
}
  ...
  <div class="space-y-2 w-full relative" :class="{ 'z-30': isOpen }">
    ...
    <div class="relative">
      ...
      <Transition name="date-dropdown">
        <div v-if="isOpen" class="absolute top-full left-0 right-0 mt-2 z-50">
          <div class="bg-white dark:bg-[#1A1D24] border border-slate-200 dark:border-white/10 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.5)] overflow-hidden">
            <!-- Header -->
