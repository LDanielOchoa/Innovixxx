...
<template>
  <header class="h-[68px] flex items-center justify-between relative z-[110] shrink-0 px-4 md:px-6 transition-all duration-500">
    <!-- Subtle top gradient for readability -->
    <div class="absolute inset-0 bg-gradient-to-b from-[#0a1628]/60 dark:from-[#0a1628]/70 via-[#0a1628]/20 dark:via-[#0a1628]/30 to-transparent pointer-events-none"></div>
    
    <!-- Left: Greeting -->
    <div class="flex items-center gap-3 animate-fade-in">
      <div class="w-10 h-10 shrink-0 rounded-[14px] bg-gradient-to-b from-white/10 to-white/5 dark:from-white/10 dark:to-white/5 border border-white/10 dark:border-white/10 flex items-center justify-center overflow-hidden">
        <img v-if="authStore.userAvatar" :src="authStore.userAvatar" class="w-full h-full object-cover" />
        <HugeiconsIcon v-else :icon="UserGroupIcon" :size="18" :stroke-width="1.8" class="text-white/60 dark:text-white/60" />
      </div>
      <div>
        <p class="text-[11px] font-medium text-white/50 dark:text-white/40 uppercase tracking-[0.15em] leading-none mb-0.5">{{ t('header.welcome') }}</p>
        <p class="text-sm font-black text-white/90 dark:text-white/80 tracking-tight leading-none">{{ authStore.userData.nombre }}</p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-3 animate-fade-in">
      ...
      <div class="flex items-center relative group-selector">
        <div class="relative">
          <button 
            @click="toggleMenu"
            class="flex items-center gap-2.5 transition-all duration-300 outline-none p-1.5 pr-3 rounded-[20px] group/btn relative overflow-hidden active:scale-[0.98] bg-white/5 dark:bg-white/5 border border-white/10 dark:border-white/10 hover:bg-white/10 dark:hover:bg-white/10"
            :class="isMenuOpen ? 'bg-white/10 dark:bg-white/10' : ''"
          >
            <div class="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/20 to-transparent group-hover/btn:animate-[shimmer_1.5s_ease-in-out]"></div>

            <div class="w-8 h-8 shrink-0 rounded-[12px] bg-white/10 dark:bg-white/10 border border-white/10 dark:border-white/10 flex items-center justify-center text-white/60 dark:text-white/60 group-hover/btn:text-[#5da6fc] transition-all duration-500 overflow-hidden relative z-10">
              <img v-if="groupStore.selectedGroup.logo" :src="groupStore.selectedGroup.logo" class="w-full h-full object-cover relative z-10" />
              <HugeiconsIcon v-else :icon="UserGroupIcon" :size="16" :stroke-width="1.8" class="relative z-10" />
            </div>

            <div class="flex items-center gap-2 relative z-10">
              <span class="text-sm font-black text-white/80 dark:text-white/70 tracking-tight leading-none pt-0.5 group-hover/btn:text-[#5da6fc] dark:group-hover/btn:text-[#5da6fc] transition-colors duration-300">
            ...
            >
              ...
              <div class="relative px-5 pt-5 pb-4 border-b border-slate-100 dark:border-white/[0.05] overflow-hidden">
                ...
                <div class="relative flex items-center justify-between mb-4">
                  <span class="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">{{ t('header.availableGroups') }}</span>
                ...
                <div v-if="groups.length > 5" class="relative group/search">
                  <div class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-500 group-focus-within/search:text-[#3b82f6] dark:group-focus-within/search:text-[#5da6fc] transition-colors z-10">
                  ...
                  <input 
                    ...
                    class="w-full bg-gradient-to-b from-white to-slate-50 dark:from-[#20242D] dark:to-[#1D1D24] border border-slate-200 dark:border-white/10 rounded-[14px] pl-10 pr-4 py-2.5 text-[12px] font-medium text-slate-700 dark:text-white outline-none placeholder:text-slate-500 dark:placeholder:text-slate-500 focus:border-[#3b82f6]/50 dark:focus:border-[#5da6fc]/50 focus:ring-2 focus:ring-[#3b82f6]/10 transition-all duration-300 shadow-[0_2px_0_#e2e8f0] dark:shadow-[0_2px_0_#1D1D24]"
                  />
                </div>
              </div>
              ...
              <div class="max-h-[300px] overflow-y-auto dropdown-scrollbar px-3 py-3 space-y-1">
                <template v-if="isRefreshing">
                  <div v-for="n in 3" :key="'skel-' + n" class="w-full px-3 py-3 animate-pulse">
                    <div class="flex items-center gap-3">
                      <div class="w-10 h-10 rounded-[14px] bg-slate-100 dark:bg-white/5"></div>
                ...
                <template v-else-if="filteredGroups.length === 0">
                  <div class="py-10 text-center flex flex-col items-center gap-3">
                    <div class="w-14 h-14 rounded-2xl bg-gradient-to-b from-white to-slate-50 dark:from-[#20242D] dark:to-[#1D1D24] flex items-center justify-center text-slate-400 dark:text-slate-600 border border-slate-200 dark:border-white/10 shadow-[0_4px_0_#e2e8f0,0_4px_12px_rgba(0,0,0,0.05)] dark:shadow-[0_4px_0_#1D1D24,0_4px_16px_rgba(0,0,0,0.3)]">
                    ...
                    <div class="space-y-1">
                      ...
                      <p class="text-[11px] font-medium text-slate-500 dark:text-slate-500">Prueba con otro término</p>
                    </div>
                  </div>
                </template>
                  ...
                  <button 
                    ...
                    :class="[
                      groupStore.selectedGroup.id === group.id 
                        ...
                        : 'bg-gradient-to-b from-white to-slate-50/90 dark:from-[#20242D]/60 dark:to-[#1A1E28]/60 border-slate-200 dark:border-white/[0.07] shadow-[0_2px_0_#e2e8f0,0_1px_4px_rgba(0,0,0,0.04)] dark:shadow-[0_2px_0_#1D1D24,0_2px_8px_rgba(0,0,0,0.2)] hover:border-slate-300 dark:hover:border-white/[0.12]'
                    ]"
                  >
                    <div
                    ...
                    <div class="flex items-center gap-3 pl-1">
                      <div
                        ...
                        :class="groupStore.selectedGroup.id === group.id
                          ...
                          : 'bg-gradient-to-b from-white to-slate-50 dark:from-[#20242D] dark:to-[#1D1D24] text-slate-500 dark:text-slate-500 border-slate-200 dark:border-white/10 shadow-[0_2px_0_#e2e8f0,0_1px_4px_rgba(0,0,0,0.05)] dark:shadow-[0_2px_0_#1D1D24,0_1px_6px_rgba(0,0,0,0.2)] group-hover/item:text-[#3b82f6] dark:group-hover/item:text-[#5da6fc]'"
                      >
                    ...
                    <div v-if="groupStore.selectedGroup.id === group.id" class="w-6 h-6 rounded-lg bg-gradient-to-b from-[#60a5fa] to-[#3b82f6] flex items-center justify-center text-white shadow-[0_3px_0_#1d4ed8,0_4px_10px_rgba(59,130,246,0.4),inset_0_1px_0_rgba(255,255,255,0.3)] border border-[#2563eb]/40 shrink-0">
