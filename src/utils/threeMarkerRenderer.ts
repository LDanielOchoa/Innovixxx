...
export class ThreeMarkerRenderer {
  ...
  private render() {
    ...
    try {
      const engine = getSharedEngine()
      // Si WebGL no está disponible el marcador queda sin flecha 3D pero no rompe nada
      if (!engine) return
      ...
      // Error inesperado, solo loguear una vez para no saturar la consola
      console.warn('Error al renderizar marcador 3D:', err)
    }
  }
